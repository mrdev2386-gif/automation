...
const getFAQs = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (data.activeOnly === true) query = query.where('isActive', '==', true);
        query = query.orderBy('createdAt', 'desc');
        if (data.limit) query = query.limit(Math.min(data.limit, 100));
...
const updateFAQ = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!faqDoc.exists) throw new functions.https.HttpsError('not-found', 'FAQ not found');
        ...
        if (data.question !== undefined) updateData.question = sanitizeInput(data.question);
        if (data.answer !== undefined) updateData.answer = sanitizeInput(data.answer);
...
const rebuildFaqEmbeddings = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!openaiApiKey) throw new functions.https.HttpsError('failed-precondition', 'OpenAI API key not configured');
        ...
        if (faqsSnapshot.empty) return { success: true, message: 'No FAQs to rebuild', count: 0 };
...
const testFaqMatch = functions.region("us-central1").https.onCall(async (data, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    if (!data.message) throw new functions.https.HttpsError('invalid-argument', 'Test message is required');
    ...
    try {
        ...
        if (faqsSnapshot.empty) return { success: true, matches: [], message: 'No FAQs configured' };
