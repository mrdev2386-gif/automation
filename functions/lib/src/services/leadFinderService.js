...
const scrapeWebsiteWithTimeout = async (url, browser, dedupeSet, userId = null, timeout = CONFIG.PAGE_LOAD_TIMEOUT_MS, config = null) => {
    ...
    try {
        ...
        page = await Promise.race([
            browser.newPage(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Page creation timeout')), timeout))
        ]);
        activeBrowserPages++;
            ...
            for (const contactPage of contactPages) {
                ...
                try {
                    ...
                    await Promise.race([
                        ...
                        new Promise((_, reject) => setTimeout(() => reject(new Error('Contact page timeout')), timeout / 2))
                    ]);
                    const content = await page.content();
                    const $ = cheerio.load(content);
                    const pageText = $('body').text();
...
const processScrapeJob = async (jobData) => {
    const { jobId, userId, websites, country, niche, createdAt } = jobData;
    ...
    try {
        ...
        for (let i = 0; i < websites.length; i++) {
            ...
            let scrapedData;
            try {
                scrapedData = await scrapeWebsiteWithTimeout(website, browser, dedupeSet, userId, config.page_load_timeout, config);
            }
            catch (browserError) {
                ...
                try {
                    ...
                    browserInfo = await browserPoolService.getBrowser(launchOptions);
                    scrapedData = await scrapeWebsiteWithTimeout(website, browserInfo.browser, dedupeSet, userId, config.page_load_timeout, config);
                }
                catch (retryError) {
...
const submitWebsites = async (userId, jobId, websites) => {
    ...
    try {
        ...
        for (let i = 0; i < validWebsites.length; i++) {
            ...
            const scrapedData = await scrapeWebsiteWithTimeout(website, browser, dedupeSet, userId, CONFIG.PAGE_LOAD_TIMEOUT_MS);
            results.push(scrapedData);
            if (scrapedData.success) {
...
//# sourceMappingURL=leadFinderService.js.map