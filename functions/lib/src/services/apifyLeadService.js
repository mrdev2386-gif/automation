...
const scrapeLinkedInCompanies = async (searchQuery, userId, maxResults = 50) => {
    try {
        ...
        // Start Apify actor
        const runResponse = await axios.post(`${APIFY_BASE_URL}/acts/${LINKEDIN_ACTOR_ID}/runs?token=${apiKey}`, {
            searchQuery,
            maxResults,
        ...
        while (attempts < maxAttempts) {
            ...
            const statusResponse = await axios.get(`${APIFY_BASE_URL}/actor-runs/${runId}?token=${apiKey}`);
            const status = statusResponse.data.data.status;
            if (status === 'SUCCEEDED') {
                // Get results
                const resultsResponse = await axios.get(`${APIFY_BASE_URL}/actor-runs/${runId}/dataset/items?token=${apiKey}`);
...
const scrapeGoogleMapsBusiness = async (searchQuery, location, userId, maxResults = 50) => {
    try {
        ...
        // Start Apify actor
        const runResponse = await axios.post(`${APIFY_BASE_URL}/acts/${GOOGLE_MAPS_ACTOR_ID}/runs?token=${apiKey}`, {
...
const discoverLeadsWithApify = async (niche, country, userId, options = {}) => {
    try {
        ...
        const { useLinkedIn = true, useGoogleMaps = true, maxResults = 50 } = options;
        const leads = [];
        // LinkedIn scraping
        if (useLinkedIn) {
...
//# sourceMappingURL=apifyLeadService.js.map