...
const captureLead = functions.region("us-central1").https.onRequest((req, res) => {
    return cors(req, res, async () => {
        ...
        try {
            const { createLead, checkDuplicate, triggerLeadAutomation, checkLeadRateLimit, isValidPhone, sanitizeString, isValidEmail } = require('./src/services/leadService');
...
const captureLeadCallable = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    const { createLead, checkDuplicate, triggerLeadAutomation, isValidEmail, isValidPhone } = require('./src/services/leadService');
