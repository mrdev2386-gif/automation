...
const deleteUser = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    const isAdmin = await isSuperAdmin(context.auth.uid);
    if (!isAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only super_admin can delete users');
    }
    // Validate input
    if (!data.userId) {
        throw new functions.https.HttpsError('invalid-argument', 'User ID is required');
...
const resetUserPassword = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    const isAdmin = await isSuperAdmin(context.auth.uid);
    if (!isAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only super_admin can reset passwords');
    }
    // Validate input
    if (!data.userId) {
        throw new functions.https.HttpsError('invalid-argument', 'User ID is required');
...
const setCustomUserClaims = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    const isAdmin = await isSuperAdmin(context.auth.uid);
    if (!isAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only super_admin can set custom claims');
    }
    // Validate input
    if (!data.userId || !data.claims) {
        throw new functions.https.HttpsError('invalid-argument', 'User ID and claims are required');
    }
    try {
        // Set custom claims
...
const getAllUsers = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    const isAdmin = await isSuperAdmin(context.auth.uid);
    if (!isAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only super_admin can view all users');
    }
    try {
...
const getUserProfile = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!userDoc.exists) {
            throw new functions.https.HttpsError('not-found', 'Access denied. Admin account not found.');
        }
...
const getDashboardStats = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    const isAdmin = await isSuperAdmin(context.auth.uid);
    if (!isAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only super_admin can view dashboard statistics');
    }
    try {
        // Get total users
        ...
        throw new functions.https.HttpsError('internal', 'Failed to fetch dashboard statistics');
    }
});
/**
...
const generateClientKey = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    const isAdmin = await isSuperAdmin(context.auth.uid);
    if (!isAdmin) {
        throw new functions.https.HttpsError('permission-denied', 'Only super_admin can generate client keys');
    }
    // Validate input
    if (!data.userId) {
        throw new functions.https.HttpsError('invalid-argument', 'User ID is required');
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to generate client key');
    }
});
module.exports = {
    createUser,
