...
const saveLeadFinderAPIKey = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (finalSerp.length > 0) updateData.serp_api_keys = finalSerp;
        if (finalApify.length > 0) updateData.apify_api_keys = finalApify;
...
const startLeadFinderCallable = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        const userData = userDoc.data();
        if (!userData.isActive) throw new functions.https.HttpsError('permission-denied', 'User account is disabled');
...
const getLeadFinderStatusHTTP = functions.https.onRequest((req, res) => {
    return cors(req, res, async () => {
        ...
        try {
            ...
            if (!jobId) return res.status(400).json({ error: 'invalid-argument', message: 'jobId is required' });
