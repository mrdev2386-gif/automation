...
const generateAIEmailDraft = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!businessName || !website || !niche) throw new functions.https.HttpsError('invalid-argument', 'Missing required fields');
...
const generateAIWhatsappMessage = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!businessName || !niche) throw new functions.https.HttpsError('invalid-argument', 'Missing required fields');
...
const qualifyAILead = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!campaignId || !leadId || !lead) throw new functions.https.HttpsError('invalid-argument', 'Missing required fields');

        let score = lead.score || 0;
        const premiumDomains = ['.com', '.io', '.co', '.biz'];
        const domain = lead.email?.split('@')[1] || '';
        if (premiumDomains.some(d => domain.endsWith(d))) score += 2;

        const freeDomains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com'];
        if (freeDomains.includes(domain)) score -= 2;
...
const updateLeadPipelineStage = functions.region("us-central1").https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!campaignId || !leadId || !newStage) throw new functions.https.HttpsError('invalid-argument', 'Missing required fields');

        const campaignDoc = await db.collection('ai_lead_campaigns').doc(campaignId).get();
        if (!campaignDoc.exists || campaignDoc.data().userId !== userId) throw new functions.https.HttpsError('permission-denied', 'Campaign not found');

        const validStages = ['new', 'qualified', 'contacted', 'responded', 'converted'];
        if (!validStages.includes(newStage)) throw new functions.https.HttpsError('invalid-argument', 'Invalid stage');
